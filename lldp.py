...
LLDP_TLV_DBP = 12 # Distance Bounding Protocol
LLDP_TLV_SEND_TIME = 9 # Added for sending timestamp
...
class lldp(packet_base.PacketBase):
    ...
    def _tlvs_valid(self):
        has_dbp = any(tlv.tlv_type == LLDP_TLV_DBP for tlv in self.tlvs)
    ...
    def _parser(cls, buf):
        ...
        while buf:
            ...
            if tlv_type not in cls._tlv_parsers:
                # Handle unknown TLVs (skip them)
                (typelen, ) = struct.unpack(LLDP_TLV_TYPELEN_STR, buf[:LLDP_TLV_SIZE])
                len_ = typelen & LLDP_TLV_LENGTH_MASK
                offset = LLDP_TLV_SIZE + len_
                buf = buf[offset:]
                continue
...
class SendTime(LLDPBasicTLV):
    """Send Time TLV encoder/decoder class
    ...
    def __init__(self, buf=None, *args, **kwargs):
        super(SendTime, self).__init__(buf, *args, **kwargs)
        if buf:
...
@lldp.set_tlv_type(LLDP_TLV_DBP)
class DBP(LLDPBasicTLV):
    ...
    def __init__(self, buf=None, *args, **kwargs):
        super(DBP, self).__init__(buf, *args, **kwargs)
        if buf:
            (self.dbp, ) = struct.unpack(
                self._PACK_STR, self.tlv_info[:self._PACK_SIZE])
        else:
            self.DBP = kwargs['dbp']
            self.len = self._PACK_SIZE
    ...
    def serialize(self):
        return struct.pack('!Hd', self.typelen, self.dbp)
